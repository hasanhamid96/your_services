...
class Persons with ChangeNotifier {
 ...
 bool isEverythingOkPerson = true;
  ...
  Future<void> fetchPersonList(int section_id,int city_id) async {
    ...
    try{
      final response = await http.get(Uri.parse( '${UserProvider.hostName}/api/users?section_id=$section_id&city_id=$city_id'),
          headers: {
...
void clearFilter(){
  ...
  bool get isNoMore{
    if(_loadedSections==null||_loadedSections.isEmpty||_loadedSections==[])
      return true;
  ...
  Future<List<Person>> searchPersonList(
    ...
    if(prevUrl!=nextUrl) {
      if (nextUrl == null) {
          url = '${UserProvider
              .hostName}/api/search?lang=ar&search=$search';
      }
  ...
  Future<bool>filterPersonList(
      {int section_id, int city_id,}) async {
  {
      if (section_id != null)
        url = '${UserProvider
            .hostName}/api/filters?city_id=$city_id&section_id=$section_id';
      if (section_id == null)
  ...
  _loadedSections=[];
    try {
      ...
      final List<Section> loadedCities = [];
      print('$data4 wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww');
