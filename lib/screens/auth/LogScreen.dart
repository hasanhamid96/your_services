...
class _LogScreenState extends State<LogScreen> {
  ...
  var third='التطبيق لا يتحمل اي مشاكل او خلافات بين الاطراف المتفقين على اي عمل كان';
  var second=' التطبيق يحملك جميع المسؤلية والمخالفات والتجاوزات القانونيه في حال تم استخدام التطبيق باي شكل مخالف او غير لائق بالمجتع او اي خدش للحياء';
       ...
       if(!isLogin)
         showCupertinoDialog(context: context, builder: (context) => StatefulBuilder(
           builder: (context, setStates) =>  CupertinoAlertDialog(
             title: Text('الشروط والاحكام',style: TextStyle(color: Colors.blue,fontFamily: 'Cairo-Regular',),),
             content: Container(
                 ...
                 child: RichText(
                   ...
                   text: TextSpan(
                       children: [
                         TextSpan(text: 'الرجاء قراءة الشروط والاحكام بالكامل لضمان حقوقك ومسؤلياتك عند قيامك باستخدام المحتوى المتوفر\n',
                           style: TextStyle(color: Colors.purple[200], fontFamily: 'Cairo-Regular',fontSize: 15,fontWeight: FontWeight.bold),),
                         TextSpan(text: '١-',
                             style: TextStyle(color: Colors.red, fontFamily: 'Cairo-Regular',fontSize: 14,fontWeight: FontWeight.normal)),
                         TextSpan(text: '$first\n',
                           style: TextStyle(color: Colors.black54, fontFamily: 'Cairo-Regular',fontSize: 14,fontWeight: FontWeight.normal),),
                         TextSpan(text: '٢-',
                             style: TextStyle(color: Colors.red, fontFamily: 'Cairo-Regular',fontSize: 14,fontWeight: FontWeight.normal)),
             ...
             actions: [
               ...
               CupertinoActionSheetAction(isDefaultAction: true,
                   onPressed: (){
                     var isValid = _key.currentState.validate();
                     if (isValid) {
                       _key.currentState.save();
                       if(isUser) {
                         ...
                         Provider.of<UserProvider>(context, listen: false).registerCraft(
                             name: name,
                             password: pass,
                             phone: phone,
                             city_id: city_id,
                           ...
                           if (value == 'true') {
                             ...
                             Navigator.of(context).pushReplacement(
                                 MaterialPageRoute(builder: (context) => BottomNavBar(islogining: true,),));
                           }
                           else{
                             setStates(() {
                               signing=false;
                             ...
                             FlushDialog.flushDialog(
                                 context, 'لم يتم عملية التسجيل', '${value==null?'يرجى محاولة مرة اخرى':value}');
                           }
                         });
                       }
                       else{
                   ...
                   Text('قبول وتسجيل',style:
                   TextStyle(color: Colors.blue, fontFamily: 'Cairo-Regular',fontSize: 13),)),
             ],
           ),
         ),);
       else {

         {

           {
             Provider.of<UserProvider>(context, listen: false)
                 .login(phone, pass,).catchError((onError){
               FlushDialog.flushDialog(
                 context,  'لم يتم عملية تسجيل الدخول','$onError',);
               setState(() {
                 signing=false;
               });
             })
               ...
               if (value == 'login success') {
                 ...
                 FlushDialog.flushDialog(
                   context,  'لم يتم عملية تسجيل الدخول','$value',);
                 setState(() {
                   signing=false;
                 });}
             });
              ...
              child: Column(
                children: [
                  ...
                  CustomPaint(
                    ...
                    child: Container(width: double.infinity,
                      child:   GestureDetector(
                        onTap: !isLogin&&isUser?()=>showCameraAndLibrary(context):null,
                        child:  Column(
                          children: [
                            SizedBox(height:50,),
                            if(!isUser)
                              CircleAvatar(
                                ...
                                child:Icon(
                                  ...
                                  size:60,color: CupertinoColors.white,),
                                radius: 70,
                                backgroundImage:_image==null?null: FileImage(_image),
                              ),
                            if(isUser)
                              CircleAvatar(
                              backgroundColor:_image!=null?Colors.transparent: CupertinoColors.systemTeal,
                              child:_image!=null?null: Icon(!isLogin?
                              CupertinoIcons.photo_camera_solid:
                              CupertinoIcons.person_crop_circle ,size:!isLogin? 45:60,color: CupertinoColors.white,),
                              radius: 70,
                              backgroundImage:_image==null?null: FileImage(_image),
                            ),
                          ],
                            ...
                            child: AnimatedSwitcher(
                              ...
                              AnimatedSwitcher(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 10,),
                                  //add pdf/image/word
                                  Text('اضافة ثبوتية   ( صورة  او  ملف ورد او  pdf ) ',
                                    textAlign: TextAlign.right,
                                    textDirection: ui.TextDirection.rtl,),
                                  InkWell(
                                    onTap:files!=null?null: pickFile,
                                    child: Container(
                  ...
                  if(!isLogin&&isUser)
                    InkWell(
                      onTap: (){
                        Navigator.of(context).pushNamed(MapScreen.routeName,arguments:getLat ).then((value) =>
                            setState(() {
                  ...
                  SizedBox(height: 20,),
                  CupertinoButton.filled(child:signing?CupertinoActivityIndicator():
                  Text(!isLogin?'تسجيل الان':'دخول',style:Theme.of(context).textTheme.headline3 ,),
                      onPressed: _onSignUp),
        ...
        child: DropdownButtonFormField<String>(
            ...
            items: items.map((String dropDownStringItem) {
              return DropdownMenuItem<String>(
                ...
                child: Text(dropDownStringItem,textAlign: TextAlign.right,textDirection: ui.TextDirection.rtl,),
              );
            }).toList(),
